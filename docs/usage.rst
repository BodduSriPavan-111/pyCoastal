...
Irregular waves in a 2D basin
-----------------------------

The example below runs a depth-averaged 2D wave model forced by an irregular
wave train at the boundary `y = 0`. The configuration is read from a YAML file,
and the solution is visualized as an animation.
...
.. code-block:: bash
   ...
   python waves2D_irregular.py

Make sure that:

- `pyCoastal` is installed in your environment  
- `configs/waves2D_irregular.yaml` exists and defines the `grid`, `physics`,
  `forcing`, `solver`, and `output` sections used in the script.
